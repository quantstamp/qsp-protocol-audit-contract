...
contract('QuantstampAudit2', function(accounts) {
  ...
  it("should audit the contract if the requestor pays", async function () {
    ...
    // request an audit
    requestCounter++;
    const result = await quantstamp_audit.requestAudit(Util.uri, price, {value : fee, from : requestor});
    ...
    assert.equal(ownerBalance.add(fee).toNumber(), (await Util.getEthBalance(owner)).toNumber());
  });
  ...
  it("should not audit unless the audit price is higher than the transaction fee", async function () {
    ...
    requestCounter++;
    Util.assertTxFail(quantstamp_audit.requestAudit(Util.uri, newFee, {value : fee, from : requestor}));
  ...
  it("should pay the auditor for their work", async function () {
    ...
    assert.equal(await Util.getReportUri(quantstamp_audit, requestId2), Util.reportUri);
  ...
  it("should log start and end timestamp when paying auditors", async function () {
    ...
    assert.equal(await Util.getReportUri(quantstamp_audit, requestId), Util.reportUri);
  ...
  it("should log an error if payment is requested for a non-pending audit", async function () {
    const price = Util.toQsp(35);
    await quantstamp_audit.requestAudit(Util.uri, price, {value : fee, from : requestor});
