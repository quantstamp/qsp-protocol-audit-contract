...
1. `npm run command -- -n=dev -a=resolve --p 1 false` calls `resolveErrorReport()` on the contract to resolve the an error report for the given requestId. If the second parameter is true, the requester is paid; otherwise the audit node is paid. 
